...
function GoogleIcon() {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24">
      ...
      <path
        fill="#34A853"
        d="M12 23c3.2 0 6-1.1 8-3l-3.7-2.9c-1.1.7-2.5 1.2-4.3 1.2-3 0-5.5-2.4-6.4-5.5L1.9 15.7C3.7 19.4 7.5 23 12 23z"
      />
    </svg>
  )
}
...
export default function SignupPage() {
  ...
  const canSubmit = fullName.trim() && email.trim() && password && accepted
    ...
    <div className="min-h-screen bg-dark-950 text-white relative overflow-hidden flex flex-col items-center justify-center">
      ...
      <div className="w-full max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 py-10 lg:py-16 min-h-screen flex items-center justify-center relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 xl:gap-16 items-center w-full">
          ...
          {/* Right Column: Centered Sign-Up Form */}
          <div className="w-full max-w-md mx-auto">
            ...
            <div className="mb-8 animate-slide-up">
              ...
              <p className="text-dark-400 text-sm sm:text-base leading-relaxed">
                Join thousands scaling their health with AI neural vision.
              </p>
            </div>

            {/* ── Form Card ── */}
            <div
              className="relative rounded-3xl overflow-hidden animate-slide-up stagger-1"
              style={{
                background:
                  'linear-gradient(165deg, rgba(13,20,36,0.95) 0%, rgba(8,13,26,0.97) 60%, rgba(6,10,20,0.98) 100%)',
                border: '1px solid rgba(255,255,255,0.08)',
                backdropFilter: 'blur(28px) saturate(180%)',
                WebkitBackdropFilter: 'blur(28px) saturate(180%)',
                boxShadow:
                  '0 30px 80px rgba(0,0,0,0.75), 0 0 60px rgba(16,185,129,0.07), inset 0 1px 0 rgba(255,255,255,0.06)',
              }}
            >
              ...
              <div
                ...
                style={{
                  background:
                    'linear-gradient(90deg, transparent 0%, rgba(52,211,153,0.7) 30%, rgba(139,92,246,0.5) 65%, transparent 100%)',
                }}
              />
              ...
              <div className="relative p-8 sm:p-10">
                ...
                <form onSubmit={handleSignup} noValidate className="flex flex-col gap-5">
                  {error && (
                    <div
                      className="p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm flex items-center gap-3 animate-pop-in"
                      role="alert"
                    >
                  ...
                  {password && (
                    <div className="p-4 rounded-2xl bg-dark-900/90 border border-white/10 animate-pop-in backdrop-blur-md">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[11px] uppercase tracking-widest text-dark-400 font-bold">
                          Password strength
                        </span>
                        <span className="text-[11px] font-bold text-emerald-400">{strength}/3</span>
                      </div>
                        ...
                        {requirements.map((_, i) => (
                          <div
                            ...
                            className={`h-1 flex-1 rounded-full transition-colors duration-300 ${
                              i < strength ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]' : 'bg-white/10'
                            }`}
                          />
                        ))}
                      </div>
                      <ul className="space-y-1.5">
                        {requirements.map((r) => (
                          <li key={r.label} className="flex items-center gap-2 text-xs">
                            ...
                            <span className={r.ok ? 'text-dark-200 font-medium' : 'text-dark-500'}>
                              {r.label}
                            </span>
                          </li>
                        ))}
                  ...
                  <label className="flex items-center gap-2.5 cursor-pointer select-none group py-1">
                    <span
                      className={`relative w-[18px] h-[18px] rounded-[6px] border transition-all duration-200 flex items-center justify-center flex-shrink-0 ${
                        accepted
                    ...
                    >
                      <Check
                        ...
                        style={{
                          ...
                          transform: accepted ? 'scale(1)' : 'scale(0.5)',
                        }}
                      />
                    </span>
                    <span className="text-xs text-dark-300 group-hover:text-white transition-colors leading-relaxed">
                      I agree to the{' '}
                      <Link
                        href="/terms"
                        className="text-emerald-300 hover:text-emerald-200 font-semibold underline underline-offset-4 decoration-emerald-400/40"
                      >
                        Terms of Service
                      </Link>{' '}
                      and{' '}
